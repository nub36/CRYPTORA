...
### Changed
- Левая лента `/signals` переименована в «Актуальные сигналы» и читает серверный `GET /api/signals?open=true`: только `ACTIVE` / `FILLED`; терминальные строки не удаляются и остаются в истории и серверной статистике.
- Строки мастер-ленты уплотнены до целевой высоты около 42 px: идентичность сигнала в первой строке, статус и время во второй, без повторения длинной фразы входа.
- Между сводкой выбранного сигнала и графиком добавлены три компактных взаимоисключающих инспектора «УРОВНИ / ИСТОРИЯ / СТАТИСТИКА» с повторным кликом для закрытия. История запрашивает терминальные строки выбранного актива через `open=false`; статистика сохраняет существующий серверный scope.
- Общий `CandleChart` при первом наборе данных нового символа/таймфрейма показывает последние 72 свечи с небольшим правым отступом. Последующие REST/WS-обновления сохраняют пользовательский zoom/pan; смена символа по-прежнему возвращает price autoscale.

### Tests
- Добавлены регрессии открытой ленты, терминальной истории, точного выбора server id/актива, трёх disclosure-контролов, стартового диапазона графика, сохранения пользовательского zoom и BTC → small-price autoscale.

### Invariants
- Математика стратегий, жизненный цикл, PostgreSQL, production settings, production signals, `enabled`, `symbols` и `scan_universe` не изменялись. Deploy не выполнялся.
