...
- **Справочные разделы:** /ecosystem — TVL сетей из DeFiLlama; /onchain — сеть Bitcoin из mempool.space; /calendar — фандинг и экспирации Binance Futures; /signals — LIVE-сетапы трёх архивных стратегий (V3.0 / V3.3 / V2.8) на закрытых свечах биржи с журналом append-only (цепочный SHA-256) и исходами по правилам стратегий; журнал пуст до первого сетапа после запуска. При отказе источника — «ИСТОЧНИК НЕДОСТУПЕН», без подстановок.
