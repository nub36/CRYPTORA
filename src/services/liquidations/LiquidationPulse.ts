...
export interface AssetImbalance {
  ...
  basedOnDemo: boolean;
  /** Какие компоненты доступны (фактические данные). */
  availableComponents: string[];
  /** Какие компоненты отсутствуют (нет данных). */
  missingComponents: string[];
  /** Процент полноты входных данных (0–100). */
...
export class LiquidationPulse {
  ...
  private static buildImbalance(
    ...
    // Track which components are actually available (not just mathematically neutral)
    const available: string[] = [];
    const missing: string[] = [];

    // Liquidation: available if actual events exist
    const hasLiquidation = liquidation.totalUsd > 0;
    if (hasLiquidation) available.push('liquidation'); else missing.push('liquidation');

    const liquidationComponent = hasLiquidation
      ? this.clamp(
          ((liquidation.shortUsd - liquidation.longUsd) / liquidation.totalUsd) * this.WEIGHTS.liquidation,
          -this.WEIGHTS.liquidation, this.WEIGHTS.liquidation
        )
      : 0;
    ...
    // OI: available only if history-derived delta is non-null (ACTUAL)
    const hasOI = futures.openInterestChange24h != null && futures.openInterestChangeSource === 'ACTUAL';
    if (hasOI) available.push('openInterest'); else missing.push('openInterest');

    const openInterestComponent = futures.openInterestChange24h != null
      ? this.clamp(futures.openInterestChange24h / this.OI_FULL_SCALE, -1, 1) * this.WEIGHTS.openInterest
      : 0;

    // Price: always available (from ticker)
    ...
    const label =
      bias === 'SHORT_SQUEEZE' ? 'Перевес вверх: шорты под давлением'
      : bias === 'LONG_SQUEEZE' ? 'Перевес вниз: лонги под давлением'
      : 'Баланс: выраженного перевеса нет';

    const basedOnDemo =
      futures.isDemo || liquidation.source === 'DEMO' || liquidation.source === 'ESTIMATED';

    const completenessPct = Math.round((available.length / 4) * 100);
