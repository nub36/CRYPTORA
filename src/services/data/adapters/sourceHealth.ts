 ...
 *
 * Проблема: REST-источники могут систематически отказывать даже через same-origin
 * gateway (сеть, WAF/гео-блок, rate limit, делистинг). Повтор каждого такого
 * запроса расходует лимит источника и не меняет результат для пользователя.
 *
...
function describeKind(kind: SourceFailureKind, detail?: string): string {
  ...
  switch (kind) {
    ...
    case 'network':
    default:
      return `сетевой отказ или источник недоступен${suffix}`;
  }
}
