...
export class IndicatorEngine {
   ...
   * Returns null if insufficient data (< 2 common observations).
   */
  public static calculateCorrelation(returnsA: number[], returnsB: number[]): number | null {
  ...
  /**
   * Beta coefficient: covariance(returnsA, returnsB) / variance(returnsB).
   * Measures sensitivity of asset A to benchmark B (e.g. BTC).
   * Returns null if insufficient data or zero benchmark variance.
   */
  public static calculateBeta(returnsA: number[], returnsB: number[]): number | null {
    const n = Math.min(returnsA.length, returnsB.length);
    if (n < 5) return null;
  ...
  /**
   * Derive simple returns from a price series: (P[i] - P[i-1]) / P[i-1]
   */
