import { TickerTick, TradeTick, OrderBookSnapshot, KlineTick, RealtimeConnectionState } from '@/types/realtime';
...
export class EventBus {
  ...
  public publishKline(kline: KlineTick): void {
    const symbolListeners = this.listeners.get(`kline:${kline.symbol}:${kline.interval}`);
    if (symbolListeners) {
      symbolListeners.forEach((cb) => cb(kline));
    }
    const wildcardListeners = this.listeners.get(`kline:${kline.symbol}:*`);
    if (wildcardListeners) {
      wildcardListeners.forEach((cb) => cb(kline));
    }
  }
