...
import type { Direction } from './coreTypes'; // source also imported unused `Candle`; dropped for CRYPTORA's noUnusedLocals — no semantic change
import type { Settings } from './legacySettings';
import {
...
export function assessRoom(
  ...
  void entry; void atr; // `void atr` added for CRYPTORA's noUnusedParameters; no semantic change
  if (targets.length === 0) {
    return {
