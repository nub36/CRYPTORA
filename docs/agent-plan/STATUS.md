...
> первым, `// R1: stop first, always.` (`v30Core.ts:manageTrade`); V3.3 — структурно так же, но комментария
> R1 нет и набор причин выхода другой; V2.8 — пары TP/SL нет вовсе, исходы только `SL|BE|TRAIL|TIMEOUT`
> (`v25Trailing.ts:simulateTrailing`). Порог зафиксирован в `outcome.sl_priority_on_ambiguous_bar = true`.
> Монитор **stateless по стратегии**: рестарт-паритет проверен на настоящем PostgreSQL для V3.0/V3.3/V2.8,
> частичный жизненный цикл (TP1 ⇒ `FILLED`, не исход) тоже. Окно 1000 баров > максимального жизненного цикла
> (≈ 113 баров у V2.8), поэтому корректный сигнал из окна не выпадает. Состояние цикла — в
...
> gross R, net R, ΣR, разрезы по стратегии и инструменту; доля успешных — только по сделкам; `null` → «—».
> Точные формулы и место результата 0 R (`losses`, отдельной категории «ничья» нет) — `docs/SIGNALS.md` §10.
