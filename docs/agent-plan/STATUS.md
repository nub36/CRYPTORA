...
> **Текущий этап:** UX / Design System Refinement (внеочередной блок владельца). Пункт **A — LIVE-first production UX** закрыт в v0.8.5; далее B (design tokens + темы), C (типографика), D (переставляемый workspace), E (визуальная полировка). Этап 6 закрыт: пункт 1 (WS-поток, v0.8.2), рабочая область Coin Detail (v0.8.3), тепловая карта плотности (v0.8.4).  
