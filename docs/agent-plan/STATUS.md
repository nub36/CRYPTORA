...
> **Текущий этап:** все пункты ТЗ, имеющие спецификацию, закрыты (Этапы 1–17, UX-цикл 1–5/A–E, Архив стратегий C1–C8, SITE_REPORT tech-debt, честность источников v0.8.25–0.8.37). Открыты только пункты, scope которых по ROADMAP определяет владелец: News/Articles (п. 6), рекламные слоты (п. 7), LLM (Этап 7), биллинг (Этап 8), а также деплой на VPS. Историю принятия пунктов см. в разделах ниже.  
...
> Прежние записи «production = v0.8.3» неверны. Версии v0.8.5–v0.8.37 на VPS **не выкатывались** (деплой не заказан).  
