...
> **Текущая версия:** v0.8.34 (журнал без засева; /signals пуст; /calendar из Binance; /onchain из mempool.space; /ecosystem из DeFiLlama; Обзор без констант, фандинг без ×1.05; ликвидации 24ч на /futures из потока; Fear & Greed из Alternative.me; корреляции по фактическим свечам; фактический Δ OI + устранён скрытый демо-фолбэк деривативов; документация + маркировка статических страниц; алерты — v0.8.23; Этап 6: система алертов Telegram/Webhook; потоки ликвидаций Bybit + OKX — v0.8.22; UX-цикл E — v0.8.21; п. 5 workspace — v0.8.20; п. 4 темы — v0.8.19; п. 3 типографика — v0.8.18; архив стратегий C1–C8 — v0.8.17)  
...
### v0.8.34 — /ecosystem из DeFiLlama
- Статический набор TVL/комиссий/TPS/адресов/стейблкоинов удалён. Теперь: TVL по 10 сетям + Δ7д из `api.llama.fi` (публичный, без ключа),
  доля L2, покрытие каталога. Отказ источника → «ИСТОЧНИК НЕДОСТУПЕН», без fallback. Живой ответ из песочницы не проверен (UNVERIFIED).
  Статической остаётся /signals.
...
- Пять справочных страниц получили явную плашку «СТАТИЧЕСКИЙ НАБОР»; ложные подписи об обновлении убраны. README актуализирован.
