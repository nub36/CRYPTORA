...
> **Текущий этап:** UX-цикл владельца (handoff §48) вместо Stage 6. Пункт **1 — LIVE-first cleanup / вывод Demo UX из production** закрыт в v0.8.6 (v0.8.5 — LIVE-first данные и честная деградация). Далее по порядку владельца: 2 системная русификация, 3 типографика + контраст + иерархия, 4 DARK/LIGHT/SYSTEM, 5 переставляемые модули workspace, 6 News/Articles, 7 рекламные слоты.  
...
#### 1.1. Режим данных
- Продуктовый режим по умолчанию — `live` (`MarketDataContext`): демо включается только явным выбором
  пользователя и сохраняется в `cryptora_data_mode`.
