...
- **Base / branch:** `4edbe168ce9ded9c236b81190072ab078ab64a49` (`origin/main`, PR #25 merge), branch `arena/01a0dd53-cryptora`; implementation commit `c3601f8d4249fd7fc9311431b37e7dde31aa2013`. Arena fixes this session branch; no work occurred on `main`, PR #25, production, or another branch.
...
- **Verification:** local `npm run typecheck` passed; local `npm test` passed (**149 files / 1650 tests**); local `npx vitest run tests/integration` passed (**12 files / 196 tests**, real embedded PostgreSQL); local `npm run build` passed; local `git diff --check` passed. Local targeted Chromium Radar E2E could not execute because the sandbox lacks `chromium_headless_shell-1243` and `npx playwright install chromium` retried with `ECONNRESET` from `cdn.playwright.dev`. GitHub Actions PR #26 installed Chromium and then passed both **Browser e2e (Chromium)** and **Typecheck + Unit + Build** for this implementation; the E2E result is therefore CI-verified, not fabricated.
...
- **Base / branch:** `a12928afa20e9421fb1b9ab6e24b0a57c0b193cd` (`origin/main`, PR #24 merge commit), branch `arena/01a0dce5-cryptora`. Arena session branch is fixed; no work was done on `main`.
- **Production #24 verification:** GitHub state verified: PR #24 is merged into `main` as `a12928afa20e9421fb1b9ab6e24b0a57c0b193cd`, and local branch starts from that commit. Direct VPS SSH check requested by the runbook failed from this sandbox (`kex_exchange_identification: Connection closed by remote host`), and HTTPS/HTTP smoke to `cryptora.duckdns.org` failed with `SSL_ERROR_SYSCALL` / empty reply; production deployment state therefore remains unverified from the sandbox. No production reset, deploy, restart, DB migration, SQL, or signal mutation was attempted.
- **Radar root cause fixed:** `/radar` no longer passively listens to whatever ticker subset other browser features happened to subscribe. In LIVE mode it reads the existing shared server Scan Universe via `src/services/signals/scanUniverse.ts` and acquires Radar-owned `RealtimeFeedManager.subscribeSymbolScoped(symbol)` leases for that bounded universe.
- **Lifecycle/refcount:** Radar universe changes are delta-applied (`[BTC, ETH] → [ETH, SOL]` releases only BTC, retains ETH, acquires SOL). Unmount and LIVE→non-live release exactly Radar leases. Existing watchlist/alerts/Coin scoped refs remain protected by `RealtimeFeedManager` refcount.
- **Warm-up UX:** UI now distinguishes `WARMING` («Радар набирает окно наблюдений…»), ready/no anomalies («В текущем LIVE-окне аномалий не обнаружено.»), filter-empty, and source-error. Header telemetry shows subscribed symbol count, warmed symbols / total, max observations / window, and WS state.
- **Read-only status:** `AnomalyEngine.getStatus()` exposes in-memory observation counts and warmed flags only. Events/history remain browser-memory; reload closes the window and requires warm-up again. Future 24/7 Radar requires a separate server-side persistence/process track.
- **Frozen scope:** anomaly thresholds, detector calculations, cooldown, severity math, strategy math, strategy settings, Scan Universe data, production signals, PostgreSQL, and Admin Scan Universe were not changed. No deploy and no merge.
- **Tests added:** unit coverage for Radar scoped universe leases, shared-subscription release protection, universe delta updates/no duplicate subscriptions, warming/ready/filter/source states, realtime event dedupe, and no demo events in LIVE; browser E2E fixture added for `/radar` bounded subscriptions → warm-up → ready → anomaly row → filters → unmount release.
- **Verification:** `npm run typecheck` passed; `npm test` passed (**146 files / 1642 tests**); `npx vitest run tests/integration` passed (**11 files / 192 tests**, real embedded PostgreSQL); `npm run build` passed; `git diff --check` passed. `npm run test:e2e` was attempted: **67 passed / 34 failed** because Playwright Chromium is not installed in the sandbox (`chromium_headless_shell-1243` missing). `npx playwright install chromium` was attempted and failed repeatedly with `ECONNRESET` from `cdn.playwright.dev`; targeted `e2e/radarLive.spec.ts` fails for the same missing-browser reason before executing product assertions. No E2E screenshot was fabricated.
