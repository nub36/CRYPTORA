...
> lookback ≤ 1000 баров 1h на группу — **веера N×свечи нет**. Исполнение и исход считает ЗАМОРОЖЕННЫЙ
> `trackPublishedSetup()` через адаптер: семантика закрытого бара, `CORRIDOR_EXPIRY_BARS = 3`,
> `TIMEOUT_BARS = 50`, комиссии 2 bps maker / 5 bps taker, сначала стоп при same-bar TP/SL, LONG fill =
> `min(open, zoneHigh)`, SHORT fill = `max(open, zoneLow)` — как в ядре; паритет зафиксирован тестом.
> Состояние цикла — в `signal_monitor_state` + `signals.monitor_*` (переживает рестарт), отказ рыночных данных
> не роняет тик и не закрывает сигнал, `stale` после двух подряд отказов. 009 не переписана; 010 аддитивная,
