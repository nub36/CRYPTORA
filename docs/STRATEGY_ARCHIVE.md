...
## 5a. UI `/strategies` — «Архив исследований → 13 версий» (C8)

`src/components/strategies/StrategyArchivePanel.tsx` поверх чистой модели `strategyArchive/presentation.ts` (`buildArchiveCards`,
`filterCounts`, `comparabilityWarnings`). Счётчик выводится из реестра (`STRATEGY_ARCHIVE_TOTAL_ROWS`), карточки — в хронологическом
порядке программы; сортировки по доходности нет намеренно. На каждой карточке раздельно: **вердикт исследования** (ВАЛИДИРОВАНО ИСТОЧНИКОМ /
TRAIN ONLY / ВАЛИДАЦИЯ ПРОВАЛЕНА / ОТКЛОНЕНО НА TRAIN / ФАЛЬСИФИЦИРОВАНО НА TRAIN) и **статус воспроизведения** (ВОСПРОИЗВЕДЕНО В CRYPTORA /
ЦЕПОЧКА ИСТОЧНИКА ПРОВЕРЕНА · НЕ ПЕРЕЗАПУСКАЛОСЬ); headline-цифры с origin `SOURCE_REPORTED`; происхождение (pin, dataset, число артефактов
sha256, расхождений, вариантов, frozen-engine); допущения; правила; оговорки; расхождения. Фильтры: Все (13) / Валидировано (2) /
Train-only (2) / Провалена валидация (1) / Отклонено (6) / Фальсифицировано (2). «Сравнить допущения» (до 3 версий) показывает только
таблицу допущений и предупреждение о несопоставимости (разные семейства, gross fees=0 vs net, lump 0.1 % V2.1, TRAIN vs VALIDATION, ТФ,
движок входов). Нет BUY/SELL/Execute/бот/ключей.

Screenshot QA (headless Chromium, `/strategies`, ширины 390/768/1024/1280/1366/1440/1920): горизонтального переполнения нет ни на одной
ширине; на 390 карточки в одну колонку, бейджи переносятся; на ≥1280 две колонки. Файлы — `screenshots/archive-c8/` (не коммитятся).
