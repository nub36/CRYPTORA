...
> **Market Radar 24/7 server monitor (2026-09-26, branch `arena/01a0dd53-cryptora`):** based on verified `origin/main` `4edbe168ce9ded9c236b81190072ab078ab64a49` (PR #25 merge). This branch migrates the authoritative Radar detector from browser lifetime to the existing backend lifecycle: server Scan Universe → one bounded server Binance ticker topology → shared frozen calculation core → persisted PostgreSQL events → server history/status APIs → browser consumer. It is **not merged, deployed, or migrated in production**. Anomaly/strategy math, Admin Scan Universe rows, production signals, production settings, and production database remain unchanged.
>
...
- **Серверный монитор открытых сигналов** (`server/services/signalMonitor/`, миграция 010): тик 30 с
  независимо от браузера, бюджет <= 500 сигналов и <= 64 групп за цикл, lookback <= 1000 баров на группу,
  размер считается по ТАЙМФРЕЙМУ ГРУППЫ (`timeframeMs()` → `ARCHIVE_TF_MS` ядра; таймфрейм из
  `signals.timeframe`) — нет веера N×свечи. Исполнение и исход считает ЗАМОРОЖЕННЫЙ `trackPublishedSetup()`
  через адаптер; паритет зафиксирован тестом. Монитор stateless по стратегии: рестарт-паритет проверен на
  настоящем PostgreSQL для V3.0, V3.3 и V2.8, частичный жизненный цикл (TP1 ⇒ `FILLED`) тоже. Окно 1000 баров
  > максимального жизненного цикла (≈ 113 баров у V2.8). Состояние
  переживает рестарт (`signal_monitor_state` + `signals.monitor_*`), отказ рыночных данных не закрывает
  сигнал и не роняет тик. Монитор стартует и при выключенных стратегиях: сопровождает уже сохранённые
  открытые сигналы, новых не создаёт.
...
- **Остаток работы (не входит в PR #16):** автоматическое серверное сопровождение позиции (по закрытым
  свечам, не tick-realtime), архивная синхронизация исходов старше окна
...
- **Не входит (остаток):** расширение `CandleChart` маркерами/линиями для других страниц, включение
  стратегий в production. Автоматическое серверное сопровождение позиции (по закрытым свечам) и честный
  статус сканирования закрыты в PR #18 (9.4).
