...
- **Не входит (остаток):** realtime-мониторинг позиции (статус остаётся «по данным последнего закрытого бара»),
  серверный перенос браузерного журнала, расширение `CandleChart` маркерами/линиями для других страниц,
  включение стратегий в production.
