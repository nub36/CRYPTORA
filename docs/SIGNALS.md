...
## 3. Отсутствие автоматического исполнения (No Auto-Execution)

- Аналитические сигналы в CRYPTORA создаются исключительно как структурированные наблюдения (Observations) и гипотезы для человека.
- Платформа **НЕ отправляет ордера на биржи** и не предоставляет функционал автоматического совершения сделок по сигналам.
- Любое торговое решение принимается пользователем самостоятельно на внешних площадках.

---

## Состояние реализации (v0.8.33)

- `SignalsAuditLedger` — append-only цепочка с настоящим SHA-256 (`src/utils/sha256.ts`, проверен на тест-векторах FIPS 180-4).
- Реестр **пуст по умолчанию**: у CRYPTORA нет фактических сетапов и трек-рекорда; иллюстративные записи не подставляются.
  Записи попадают только через `append()`; редактирование задним числом ломает `verifyIntegrity()`.
- Страница `/signals` показывает методологию и блок «Реестр пуст: фактических сетапов нет».
