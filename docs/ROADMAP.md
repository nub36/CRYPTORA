# ROADMAP — Дорожная карта развития CRYPTORA

> **Генеральный план поэтапной реализации платформы**  
> ⚠️ **КЛЮЧЕВОЙ ИНВАРИАНТ:** CRYPTORA проектируется строго как **Crypto Market Intelligence Terminal** (Рынок → Данные → Аналитика → Наблюдения → Решение пользователя). Торговый функционал, исполнение ордеров, торговые боты, автотрейдинг, кастоди и торговые API-ключи полностью исключены из концепции и всех этапов дорожной карты.

---

## Этап 1: Visual Foundation & Demo Data (ЗАВЕРШЕН)
- [x] Создание полной проектной памяти и архитектурной документации (`docs/`, `docs/agent-plan/`, `AGENTS.md`).
- [x] Настройка стека: React 19, TypeScript, Vite, Tailwind CSS, Lucide React, Zod, Vitest, Playwright.
- [x] Реализация интерфейсов `MarketDataProvider` и детерминированного слоя `DemoMarketDataProvider`.
- [x] Полноценная оболочка терминала высокой плотности с навигацией и бейджами демо-режима.
- [x] Главный командный центр (Overview): макро-показатели, график BTC, деривативы, ликвидации, радар, топ movers.
- [x] Таблица рынка (/market): 30 монет, сортировка, фильтры по секторам, поиск, спарклайны.
- [x] Страница инструмента (/coin/:symbol): мульти-таймфреймный график, показатели деривативов, стакан/пары, индикаторы, радар.
- [x] Раздел фьючерсов (/futures) и раздел ликвидаций (/liquidations) с методологическим дисклеймером (`Actual != Estimated`).
- [x] Интерактивный скринер (/screener) с работающими фильтрами по demo data.
- [x] Поток аномалий Market Radar (/radar).
- [x] Тепловые карты рынка (/heatmaps) с переключением режимов.
- [x] Работающие калькуляторы Position Size и PnL с тестами (/tools).
- [x] Честные превью разделов Strategy Lab (/strategies) и Signals (/signals).
- [x] Watchlist (сохранение в LocalStorage) и Alerts preview.
- [x] Покрытие юнит-тестами Vitest и Playwright E2E тестами (30 проверок).
- [x] Приемочный аудит и фиксация базовой версии v0.1.0.

---

## Этап 2: Spot Market Data (Binance & KuCoin) (ЗАВЕРШЕН)
- [x] Публичные спотовые данные без API-ключей с Binance и KuCoin.
- [x] Контролируемый начальный universe из 25 криптоактивов.
- [x] Централизованный канонический Asset Registry (канонический актив → символ Binance → символ KuCoin).
- [x] Zod-схемы валидации DTO и слой нормализации в каноническую модель CRYPTORA.
- [x] Адаптеры `BinanceSpotAdapter` (primary) и `KuCoinSpotAdapter` (secondary/fallback).
- [x] Фиксация источника и времени каждого значения (Provenance: exchange, market, symbol, timestamp).
- [x] `LiveMarketDataProvider` с изолированными режимами DEMO и LIVE (без подмешивания фиктивных данных при сбоях сети).
- [x] Тесты с детерминированными фикстурами реальных ответов API (без сетевой зависимости в CI).

## Этап 3: Realtime WebSockets & Anomaly Engine (ЗАВЕРШЕН)
- [x] WebSocket шлюз для котировок, публичных сделок и стаканов L2.
- [x] Математический движок детекции аномалий (Z-Score объемов, всплески активности) для Market Radar.

## Этап 4: Historical Time-Series & Deep Indicators (ЗАВЕРШЕН)
- [x] Развертывание хранилища временных рядов (интерфейс TimeSeriesRepository, кэш и детекция пробелов).
- [x] Высокоточный индикаторный движок (SMA, EMA, Wilder's RSI, MACD, Bollinger Bands, ATR, VWAP, Volume Profile, CVD).
- [x] Углубленные карточки активов со стаканом Level 2 и динамической глубиной.

## Этап 5: Derivatives & Aggregated Futures (ЗАВЕРШЕН)
- [x] Сбор и агрегация метрик фьючерсов (Open Interest, Funding rate, Basis, Liquidation estimates).
- [x] Адаптер Binance Futures (`BinanceFuturesAdapter`) без ключей с валидацией Zod.
- [x] Расчетный движок деривативов (`DerivativesEngine`: APR, Basis, Contango/Backwardation, OI USD).
- [x] Страница фьючерсов (/futures) с живыми агрегатами и фильтрацией Short Squeeze Watch.

## Этап 6: Liquidation Pipeline & Risk Heatmaps (ТЕКУЩИЙ ЭТАП)
- [x] WebSocket-поток фактических биржевых ликвидаций (Binance USD-M `!forceOrder@arr`, v0.8.2) и честная деградация при недоступности потока.
- [x] Приём фактических потоков ликвидаций других бирж (Bybit V5 `allLiquidation`, OKX `liquidation-orders`, v0.8.22) — доли считаются только по подключённым потокам; неподключённые не оцениваются.
- [x] Аналитическая тепловая карта плотности ликвидаций (2D-визуализация цена × время поверх расчетной модели `MODEL / ESTIMATED`, v0.8.4) — секция «Тепловая карта плотности ликвидаций (Price × Time)» на `/liquidations`; уровни 1/плечо по тирам 10x–100x, детерминированная нормировка, явный провенанс входных свечей.
- [x] Расширенная система алертов (v0.8.23): оценка правил по фактическим тикам (WS) и фандингу (REST), история срабатываний, каналы В приложении / Браузер / Telegram Bot API / Webhook с журналом доставки (DELIVERED / SENT_UNCONFIRMED / FAILED); лимит тарифа; настройки каналов только в браузере пользователя, без серверного посредника.

## UX / Design System Refinement (внеочередной блок владельца, v0.8.5+)
- [x] **A. LIVE-first production UX** (v0.8.5): фактический источник по умолчанию, честная деградация вместо глобального DEMO, каталог символов вместо демо-датасета в runtime-компонентах, режимные подписи страниц и удаление фиксированных демо-чисел из Обзора.
- [x] **B. Semantic design tokens + темы DARK / LIGHT / SYSTEM** (v0.8.19) (переключение, `prefers-color-scheme`, сохранение выбора, темизация графиков).
- [x] **C. Типографика** (v0.8.18): современный sans для UI/навигации, mono только для чисел/тикеров, сокращение ALL CAPS, минимальный кегль 12px на desktop, `tabular-nums`.
- [x] **D. Переставляемый workspace** (v0.8.20, Coin Detail): крупные модули, drag-handle + keyboard-альтернатива, сохранение раскладки с версионированной схемой и кнопкой сброса.
- [x] **E. Визуальная полировка** (v0.8.21): контраст, иерархия, пустые зоны, скриншот-QA 390–1920 в обеих темах.

## UX-цикл владельца (handoff §48) — вместо продолжения Stage 6
- [x] **1. LIVE-first cleanup** (v0.8.5 + v0.8.6): фактический источник по умолчанию, честная деградация,
  Demo UX выведен из production (переключатель режима удалён, LIVE/WS — только статус соединения).
- [x] **2. Системная русификация** (v0.8.7): сняты англо-дубли в скобках, латинские заголовки блоков
  (Price × Time, Actual Event Log, Estimated Liquidation Levels, Long / Short), названия разделов навигации;
  латиница только у общепринятых аббревиатур и провенанса (`docs/DESIGN_SYSTEM.md` §11, словарь `src/utils/labels.ts`).
- [x] **3. Typography + contrast + hierarchy** (v0.8.18): sans для UI, mono только для чисел/тикеров, меньше ALL CAPS,
  единые уровни заголовков; компактнее дисклеймер «ACTUAL ≠ ESTIMATED» без потери смысла; компактные
  информативные пустые состояния.
- [x] **4. DARK / LIGHT / SYSTEM** (v0.8.19) (семантические токены, `prefers-color-scheme`, темизация графиков).
- [x] **5. Переставляемые модули workspace** (v0.8.20, Coin Detail): drag-handle + keyboard-альтернатива,
  версионированная схема раскладки, кнопка сброса. Инварианты heatmap (2D price × time, density clusters,
  current-price marker, MODEL / ESTIMATED, провенанс входа) сохраняются.
- [x] **6. News / Articles** (v0.8.39, scope владельца: собственные статьи в markdown, без внешних лент) — `/articles`, `content/articles/`.
- [x] **7. Рекламные слоты** (v0.8.39, scope владельца: слоты «Sponsored / Partner» из `content/sponsor-slots.json`, без ad-сетей; пустой конфиг = не отображаются).

## Этап 7: AI Analyst Explanation Layer
- [x] Интеграция LLM строго поверх структурированных фактов аналитического движка (v0.8.38): серверный `/api/ai/explain`, ключ в env, страж заземления; без ключа — детерминированный движок.

## Этап 8: Monetization & Pro Subscriptions
- [ ] Подключение биллинга (отложено владельцем 2026-09-17; UI честно сообщает «Биллинг не подключён», v0.8.37).
