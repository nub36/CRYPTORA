 ...
 * «совершилась сделка» считаются раздельно, знаменатель win rate — завершённые
 * сделки с ИЗВЕСТНЫМ результатом (wins + losses + breakEven), R не
 * пересчитывается (берётся как его посчитало ядро).
 *
 * Классификация завершённой сделки (взаимоисключающие корзины):
 *   wins      — result_r > 0
 *   losses    — result_r < 0
 *   breakEven — result_r = 0   (НЕ убыток)
 *   unrated   — result_r IS NULL (не победа/поражение/ничья)
 * NULL никогда не трактуется как 0.
 */
